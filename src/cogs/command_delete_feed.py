...
class CommandDeleteFeed(CommandsCog):
    ...
    async def command_delete_feed(self, ctx, channel: Union[TextChannel, DMChannel], link_rss: Optional[str] = None):
    ...
    async def _delete_feed(self, source, channel, user, link_rss: Optional[str] = None):
        try:
            ...
            feed_bll = FeedBLL()
            emty_bll = EmtyBLL()
            channel_id = channel.id if isinstance(channel, TextChannel) else str(user.id)
        
            if isinstance(channel, (TextChannel, DMChannel)):
                channel_id = str(channel.id)
            else:
                logger.warning(f"Invalid channel type: {type(channel)}, using user ID instead.")
